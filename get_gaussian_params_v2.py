...
# --- 2. SINGLE SUBJECT WORKER (called by each parallel process) ---
def process_subject(args):
    file_path, fixed_means, n_components, threshold, output_dir = args
...
def main():
    ...
    if os.path.exists(centers_file):
        ...
        print(f"[*] Centers not found. Fitting average brain ({average_brain_file})...")
    ...
    # --- STEP C: PROCESS IN PARALLEL ---
    worker_args = [
        (fp, fixed_means, n_components, threshold, output_dir)
    ...
    with Pool(processes=n_workers) as pool:
        for result in pool.imap_unordered(process_subject, worker_args):
            ...
            rate = completed / elapsed  # subjects per second
